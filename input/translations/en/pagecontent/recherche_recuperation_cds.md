...
This flow is based on the query of the IHE transaction "Search for Care Team" [PCC-46] from the DCTM profile based on the FHIR "search" interaction.
...
These search criteria were defined in the [Functional Specifications](specifications_fonctionnelles.html).
...
This flow is based on the query of the IHE transaction "Retrieve Care Team" [PCC-47] from the DCTM profile. Retrieving a CareTeam resource corresponding to a specified identifier is based on the FHIR "read" interaction. Flow 2b is therefore an HTTP GET request accompanied by the "id" parameter.

Retrieving a CareTeam resource corresponding to a logical identifier should be possible by relying on the FHIR "vread" interaction, which takes into account the version of the resource, and on the "history" interaction, which allows requesting the retrieval of a specific version of the resource (history-instance and history-type should be offered).
