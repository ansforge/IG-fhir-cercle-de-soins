...
* The CareTeam resource (profile [CDS_IHECareTeam](StructureDefinition-cds-ihe-careteam.html)),
...
* The resource(s) referenced as members of the Care Team, and those they reference, among the following resources:
  ...
  * RelatedPerson (profile [CDS_FrRelatedperson](StructureDefinition-cds-fr-related-person.html)) to represent a third party,
...
These resources are encapsulated in a "Bundle" resource of type "transaction" compliant with the profile [CDS_BundleTransactionCreation](StructureDefinition-cds-bundle-transaction-creation.html). The Bundle contains at least one CareTeam resource. This Bundle constitutes the body of the HTTP POST request.
...
If the transaction has been correctly performed and therefore the creation of the care team is correctly performed, an HTTP 200 OK code is returned. A transaction-response type Bundle must be returned in the response body. The latter must contain the resources as they were created by the manager or, at a minimum, the logical identifiers of the resources that were assigned by the manager (in Bundle.entry.fullUrl and/or Bundle.entry.resource.id).
Otherwise, an HTTP 500 Internal Server Error code is returned with an OperationOutcome resource containing the details of the errors and warnings resulting from the processing of the Bundle entries.

### Updating care teams

The Bundle [CDS_BundleTransactionMAJ](StructureDefinition-cds-bundle-transaction-maj.html) can contain exactly the same resources as the [CDS_BundleTransactionCreation](StructureDefinition-cds-bundle-transaction-creation.html). The Bundle contains at least one CareTeam resource; regarding the actors, only the resource(s) that need to be created or updated are included in the bundle. This Bundle constitutes the body of the HTTP POST request.
